...
console.log(`${BLUE}🧪 ENCODE Structured Content Regression Tests${RESET}`);

const toolExpectations = [
  {
    path: 'src/tools/code-mode.ts',
    required: ['createSearchTool', 'createExecuteTool', 'encodeCatalog'],
  },
  {
    path: 'src/tools/query-data.ts',
    required: ['createQueryDataHandler', 'encode_query_data', 'ENCODE_DATA_DO'],
  },
  {
    path: 'src/tools/get-schema.ts',
    required: ['createGetSchemaHandler', 'encode_get_schema', 'ENCODE_DATA_DO'],
  },
  {
    path: 'src/spec/catalog.ts',
    required: ['encodeCatalog', 'encodeproject.org'],
  },
];
...
assertContains('src/index.ts', indexContent, 'EncodeDataDO', 'index.ts exports EncodeDataDO');
...
console.log(`\n${GREEN}✅ ENCODE structured content regression tests passed.${RESET}`);
